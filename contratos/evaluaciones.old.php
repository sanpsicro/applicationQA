...
$pag = ($_GET['pag']); 
if (!isset($pag)) $pag = 1;
$result = mysql_query("SELECT COUNT(*) from general left join evaluaciones on (general.id = evaluaciones.general) $condicion", $link); 
list($total) = mysql_fetch_row($result);
$tampag = $show;
$reg1 = ($pag-1) * $tampag;
$result = mysql_query("SELECT general.id as milene,general.idEmpleado,general.contrato,general.fecha_recepcion,general.evaluado,evaluaciones.promedio from general left join evaluaciones on (general.id = evaluaciones.general) $condicion order by $sort LIMIT $reg1, $tampag", $link); 
$_GET["accela"]=$accela;
$_GET["quest"]=$quest;
$_GET["sort"]=$sort;
$_GET["show"]=$show;
