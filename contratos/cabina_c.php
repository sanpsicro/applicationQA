...
$result = mysql_query("SELECT * from usuarios_contrato where clave = '$clave'",$db);
...
$servicio_vehiculo=mysql_result($result,0,"servicio");
...
$db = mysql_connect($host,$username,$pass);
mysql_select_db($database,$db);
$result = mysql_query("SELECT * from TipoVenta where idVenta = '$tipoventa'",$db);
...
$db = mysql_connect($host,$username,$pass);
mysql_select_db($database,$db);
$result = mysql_query("SELECT * from Poliza where idPoliza = '$pre_idPoliza'",$db);
$idcliente=mysql_result($result,0,"idCliente");

$db = mysql_connect($host,$username,$pass);
mysql_select_db($database,$db);
$result = mysql_query("SELECT * from Cliente where idCliente = '$idcliente'",$db);
$nombre_cliente=mysql_result($result,0,"nombre");
...
$nombre_servicio=mysql_result($result,0,"servicio");
$tipo_servicio=mysql_result($result,0,"tipo");
...
$db = mysql_connect($host,$username,$pass);
mysql_select_db($database,$db);
$result = mysql_query("SELECT * from general order by id desc limit 1",$db);
...
if($checa_array===FALSE){} else{echo '<td bgcolor="#CCCCCC"><strong>Ubicaci&oacute;n Estado:</strong></td>
...
if($checa_array===FALSE){} else{echo '<td bgcolor="#CCCCCC"><strong>Ubicaci&oacute;n Municipio:</strong></td>
         <td bgcolor="#CCCCCC">';
...
if($checa_array===FALSE){
#CIXVERICL
echo'<tr><td colspan="6" bgcolor="#999999"><span class="style1">Informaci&oacute;n del Veh&iacute;culo</span></td></tr><tr>';
$cuenta_campos=0;
...
if($checa_array===FALSE){} else{echo '         <td bgcolor="#CCCCCC"><strong>Placas:</strong></td><td bgcolor="#CCCCCC"><input name="placas" type="text" id="placas" size="20" onattrmodified="g(this)" onpropertychange="g(this)" onKeyDown="f(this)" onKeyUp="f(this)" onBlur="f(this)" onClick="f(this)" value="'.$placas.'"/></td>';
		 $cuenta_campos=$cuenta_campos+1;
