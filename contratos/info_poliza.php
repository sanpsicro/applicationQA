...
$db = mysql_connect($host,$username,$pass);
mysql_select_db($database,$db);
$result = mysql_query("SELECT * from Estado where idEstado = '$aseg_estado'",$db);
if (mysql_num_rows($result)){ 
$aseg_estado=mysql_result($result,0,"NombreEstado");
}
$db = mysql_connect($host,$username,$pass);
mysql_select_db($database,$db);
$result = mysql_query("SELECT * from Municipio where idMunicipio = '$aseg_municipio'",$db);
if (mysql_num_rows($result)){ 
$aseg_municipio=mysql_result($result,0,"NombreMunicipio");
}
$db = mysql_connect($host,$username,$pass);
mysql_select_db($database,$db);
$result = mysql_query("SELECT * from Colonia where idColonia = '$aseg_colonia'",$db);
if (mysql_num_rows($result)){ 
...
<td bgcolor="#FFFFFF"><strong>Asegurado:</strong> <? echo $aseg_asegurado;?></td>
      <td bgcolor="#FFFFFF"><strong>Y/O:</strong> <? echo $asegurado_y_o;?></td>
		...
		<? if(empty($NO_EDITAR)):?>
          <td align="right" bgcolor="#FFFFFF"><strong>[ <a href="javascript:FAjax('edita_infopoliza.php?id=<? echo $id;?>&flim-flam=new Date().getTime()','infopoliza','','get');">Editar</a> ]</strong></td>
		  <? endif; ?>
        </tr>
      </table>