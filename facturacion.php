...
$pag = ($_GET['pag']); 
if (!isset($pag)) $pag = 1;
$result = mysqli_query("SELECT COUNT(*) FROM facturas left join Cliente on (facturas.cliente = Cliente.idCliente) $condicion", $link); 
list($total) = mysql_fetch_row($result);
$tampag = $show;
$reg1 = ($pag-1) * $tampag;
$result = mysqli_query("SELECT facturas.id as factid, facturas.cliente, facturas.factura, facturas.fecha, facturas.total, facturas.status as statusfactura, Cliente.nombre FROM facturas left join Cliente on (facturas.cliente = Cliente.idCliente) $condicion order by $sort  
